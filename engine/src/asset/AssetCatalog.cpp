...
static Detail::Result<std::string, AssetError>
ReadAllText(std::string_view path) {
    ...
    ss << ifs.rdbuf();
    return Detail::Result<std::string, AssetError>::Ok(ss.str());
}
...
AssetCatalog::LoadFromFile(std::string_view catalogJsonPath,
    ...
    if (!textR) return Detail::Result<void, AssetError>::Err(std::move(textR.error()));

    auto rawR = parser.Parse(textR.value(), catalogJsonPath);
    if (!rawR) return Detail::Result<void, AssetError>::Err(std::move(rawR.error()));
...
AssetCatalog::BuildFromRaw_(const std::vector<Catalog::RawCatalogEntry>& raw,
    ...
    for (const auto& r : raw) {
        ...
        if (!rp) {
            ...
            return Detail::Result<void, AssetError>::Err(AssetError::Make(
