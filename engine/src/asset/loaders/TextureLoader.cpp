...
static Detail::Result<std::shared_ptr<TextureAsset>, AssetError>
DecodePPM(Detail::ConstSpan<std::byte> bytes, const Loading::LoadContext& ctx) {
    const char* p = reinterpret_cast<const char*>(bytes.data());
    const char* end = p + bytes.size();

    if (end - p < 2) {
        return Detail::Result<std::shared_ptr<TextureAsset>, AssetError>::Err(
    ...
    if (isP6) {
        ...
        return Detail::Result<std::shared_ptr<TextureAsset>, AssetError>::Ok(std::move(tex));
    }
...
TextureLoader::Load(Detail::ConstSpan<std::byte> bytes, const Loading::LoadContext& ctx) {
    ...
    if (!decoded) {
        return Detail::Result<Core::AnyAsset, AssetError>::Err(std::move(decoded.error()));
    }
