...
public:
    ...
    Detail::Result<AssetHandle, AssetError> Load(const AssetId& id, const AssetRequest& request);
...
private:
    ...
    Detail::Result<ResolvedEntry, AssetError> ResolveEntry_(const AssetId& id, const AssetRequest& req);

    // Record 取得/作成
    Core::AssetRecord& GetOrCreateRecord_(const AssetId& id, const ResolvedEntry& e);

    // 実ロード（Sync）
    Detail::Result<void, AssetError> DoLoadSync_(Core::AssetRecord& rec, const ResolvedEntry& e, const AssetRequest& req);
